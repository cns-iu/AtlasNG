...
export function isUrlActive(targetUrl: string, currentUrl: string, options: IsActiveMatchOptions): boolean {
  ...
  const targetPathname = decodeURIComponent(target.pathname);
  const currentPathname = decodeURIComponent(current.pathname);
  ...
  return (
    sameOrigin &&
    comparePathsWithMatrixParams(targetPathname, currentPathname, options) &&
    compareQueryParams(target.searchParams, current.searchParams, options) &&
    compareFragments(targetHash, currentHash, options)
  );
}
